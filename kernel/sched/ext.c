...
static struct scx_dispatch_q *find_global_dsq(struct scx_sched *sch, s32 task_cpu)
{
	return &sch->pnode[cpu_to_node(task_cpu)]->global_dsq;
}
						    ...
						    s32 task_cpu)
{
	...
	if ((dsq_id & SCX_DSQ_LOCAL_ON) == SCX_DSQ_LOCAL_ON) {
		...
		if (!ops_cpu_valid(sch, cpu, "in SCX_DSQ_LOCAL_ON dispatch verdict"))
			return find_global_dsq(sch, task_cpu);

		return &cpu_rq(cpu)->scx.local_dsq;
	}

	if (dsq_id == SCX_DSQ_GLOBAL)
		dsq = find_global_dsq(sch, task_cpu);
	else
				  ...
				  struct task_struct *p, u64 enq_flags)
{
	...
	if (src_rq != dst_rq &&
	    unlikely(!task_can_run_on_remote_rq(sch, p, dst_rq, true))) {
		dispatch_enqueue(sch, find_global_dsq(sch, task_cpu(p)), p,
				 enq_flags | SCX_ENQ_CLEAR_OPSS);
		return;
	}
			       ...
			       u64 reenq_flags)
{
	...
	if (dsq->id == SCX_DSQ_LOCAL) {
		...
		scoped_guard (raw_spinlock_irqsave, &rq->scx.deferred_reenq_lock) {
			if (list_empty(&sch_pcpu->deferred_reenq_local_node))
				list_move_tail(&sch_pcpu->deferred_reenq_local_node,
					       &rq->scx.deferred_reenq_locals);
			sch_pcpu->deferred_reenq_local_flags |= reenq_flags;
		}
		...
		scoped_guard (raw_spinlock_irqsave, &rq->scx.deferred_reenq_lock) {
			if (list_empty(&dsq_pcpu->deferred_reenq_user_node))
				list_move_tail(&dsq_pcpu->deferred_reenq_user_node,
					       &rq->scx.deferred_reenq_users);
			dsq_pcpu->deferred_reenq_user_flags |= reenq_flags;
		}
...
{
	struct scx_sched_pcpu *sch_pcpu, *sch_pcpu_n;

	lockdep_assert_rq_held(rq);

	guard(raw_spinlock)(&rq->scx.deferred_reenq_lock);

	list_for_each_entry_safe(sch_pcpu, sch_pcpu_n, &rq->scx.deferred_reenq_locals,
				 deferred_reenq_local_node) {
		u64 reenq_flags = 0;

		swap(sch_pcpu->deferred_reenq_local_flags, reenq_flags);
		list_del_init(&sch_pcpu->deferred_reenq_local_node);

		reenq_local(sch_pcpu->sch, rq, reenq_flags);
	}
}
		...
		scoped_guard (raw_spinlock, &rq->scx.deferred_reenq_lock) {
			struct scx_dsq_pcpu *dsq_pcpu =
				list_first_entry_or_null(&rq->scx.deferred_reenq_users,
							 struct scx_dsq_pcpu,
							 deferred_reenq_user_node);
			if (!dsq_pcpu)
				return;

			dsq = dsq_pcpu->dsq;
			swap(dsq_pcpu->deferred_reenq_user_flags, reenq_flags);
			list_del_init(&dsq_pcpu->deferred_reenq_user_node);
		}
		    ...
		    struct scx_sched *sch)
{
	...
	for_each_possible_cpu(cpu) {
		...
		pcpu->dsq = dsq;
		INIT_LIST_HEAD(&pcpu->deferred_reenq_user_node);
	}
...
static void exit_dsq(struct scx_dispatch_q *dsq)
{
	...
	for_each_possible_cpu(cpu) {
		 ...
		 */
		if (WARN_ON_ONCE(!list_empty(&pcpu->deferred_reenq_user_node))) {
			guard(raw_spinlock_irqsave)(&rq->scx.deferred_reenq_lock);
			list_del_init(&pcpu->deferred_reenq_user_node);
		}
	}
...
__bpf_kfunc void scx_bpf_dsq_insert(struct task_struct *p, u64 dsq_id,
				    ...
				    const struct bpf_prog_aux *aux__prog)
{
	scx_bpf_dsq_insert___v2(p, dsq_id, slice, enq_flags, aux__prog);
}
 ...
 * future.
 */
__bpf_kfunc void scx_bpf_reenqueue_local___v2(const struct bpf_prog_aux *aux__prog)
{
	scx_bpf_dsq_reenq(SCX_DSQ_LOCAL, 0, aux__prog);
}
...
BTF_ID_FLAGS(func, scx_bpf_dsq_reenq)
