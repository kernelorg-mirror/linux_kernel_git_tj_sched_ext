 ...
 * bpf_arena_alloc_pages_sleepable() call and is registered with the
 * kernel-side mapping address. The matching BPF-side uaddr is recovered by
 * subtracting the arena's kern_vm_start.
 *
...
void *scx_arena_alloc(struct scx_sched *sch, size_t size, u32 *uaddr_out)
{
	...
	/*
	 * uaddr = kern_va - kern_vm_start. (u32)kern_va alone would be wrong:
	 * get_vm_area() does not 4 GiB-align kern_vm->addr, so the low 32 bits
	 * of kern_vm_start are not zero.
	 */
	*uaddr_out = kern_va - bpf_arena_map_kern_vm_start(sch->arena_map);
	return (void *)kern_va;
}
